...
#define LIST_OF_UNITS                                                          \
  ...
  X(VAR, var, "VAR", Reactive_Power, "volt amperes reactive")          \
...
private:
  ...
  int8_t
      f_{}; // Since SIUnits are also used for conversions of values, not just
            // unit tracking, this means that the offset units (c,f) must be
            // treated as distinct units. E.g. if you calculated m3*k (and
            // forget the m3 and k value) then you cannot convert this value
            // into m3*c since  the offset makes the calculation depend on the k
            // value, which we no longer know. But kw*h can be translated into
            // kw*s since scaling (*3600) can be done on the calculated value
            // without knowing the h value. Therefore we have to treat k, c and
            // f as distinct units. I.e. you cannot add m3*k+m3*f+m3*c.
  int8_t month_{}; // Why month and year here instead of using s? Because they
                   // vary in length and thus cannot
  int8_t year_{};  // be auto-converted to seconds, ie you cannot add/subtract
                   // months/years without knowing what you add to.
  int8_t unix_timestamp_{}; // Why not s? Because point in time is referenced
                            // agains an offset. UT is 1970-01-01 01:00.

  // If exponents have over/underflowed or if multiple of (k,c,f)
  // (month,year,unix_timestamp) are set, then the SIExp is not valid any more!

  bool invalid_ = false;
};

enum class MathOp { ADD, SUB };

struct SIUnit {
  // Transform a double,double,uint64_t into an SIUnit.
  // The exp can be created compile time like this: SIUNIT(3.6E6, 0,
  // SI_KG(1)|SI_M(2)|SI_S(-2)) which is kwh.
  ...
  SIUnit(std::string s);
  // Return the known unit that best matches the SIUnit, or Unit::Unknown if
  // none.
  Unit asUnit() const;
  // Return the known unit that best matches the SIUnit and the quantity, or
  // Unit::Unknown if none.
  ...
  bool sameExponents(SIUnit &to_siunit) const {
    return exponents_ == to_siunit.exponents_;
  }
  // Convert value from this unit to another unit and store it in out. Return
  // false if conversion is impossible!
  bool convertTo(double left, const SIUnit &out_siunit, double *out) const;
  // Do a math op. Store the resulting unit and value into the destination
  // pointers. Return false if the addion cannot be performed.
  bool mathOpTo(MathOp op, double left, double right,
                const SIUnit &right_siunit, SIUnit *out_siunit,
                double *out) const;
...
// not a valid conversion, but permitted to work around limitations in the mbus
// protocol usage.
